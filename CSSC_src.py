...
class CSSC:
    ...
    def get_TB_offsets(self):
        ...
        n_f_fit = 15        # number of sondes required to accept linear fit; if not surpassed, use bias instead
        for date, DS_date in TB_stat_DS_grouped:
            # use the daily bias if sufficient sondes were available, otherwise, use no bias (or the mean bias?)
            bias_daily.loc[{'date': date}] = xr.where(N_daily.loc[{'date': date}] >= n_f_bias, 
                                                        DS_date.bias.isel(time=DS_date.date==date)[0], 0.0)
            ...
            if np.count_nonzero(N_daily.loc[{'date': date}] < n_f_bias) > 0: # then more than 1 frequency has got less than n_f_bias clear sky dropsondes
                print(f"{date} : {np.count_nonzero(N_daily.loc[{'date': date}] < n_f_bias)} number of freqs do not have " +
                        "sufficient clear sky dropsondes available. No bias estimation; bias is set to 0!")
            ...
            STAT_DS = xr.Dataset({'bias':       (['date', 'freq'], np.reshape(bias_daily.loc[{'date': date}].values, (1,self.n_freq)),
                                                {'long_name': f"Estimated daily bias (daily_bias) for days with sufficient sondes (>= {n_f_bias})",
                                                'units': "K"}),
                                    ...
                                    'mean_bias': (['freq'], bias_mean.values,
                                                ...
                                                'comment': ("This value could also be an option for the bias correction on days with insufficient sondes " +
                                                            f"where N < {n_f_bias}."),
                                                'units': "K"}),
            ...
            # set attributes:
            bias_description = (f"A bias of 0 is used if less than {n_f_bias} clear sky " +
                                "sondes were detected. Subtract this number from TB measurements to apply offset " +
            ...
            lin_fit_descript = ("Use slope and offset to apply a correction that is either linear or offset only (i.e., slope==1.): " +
                                ...
                                "clear sky sondes must be available.")
            ...
            STAT_DS.attrs['title'] = ("HALO-(AC)3 HALO brightness temperature (TB) correction derived from " +
                                        "comparisons between PAMTRA-simulated sondes and HAMP microwave radiometer observation")
